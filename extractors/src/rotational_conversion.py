...
def batch_angle_axis_to_euler(angle_axis):
    """
    Convert a batch of angle-axis representations to Euler angles.

    Parameters:
    - angle_axis: torch.Tensor of shape (N, 3) representing N angle-axis rotations.

    Returns:
    - torch.Tensor of shape (N, 3) representing the Euler angles for each rotation.
    """
    # Convert angle-axis to rotation matrix
    rot_mats = batch_axis2matrix(angle_axis)

    # Convert rotation matrix to Euler angles
    euler_angles = batch_matrix2euler(rot_mats)
...
def quaternion_to_euler(quat):
    ...
    Parameters:
    - quat: torch.Tensor of shape (N, 4) representing N quaternions.

    Returns:
    - torch.Tensor of shape (N, 3) representing the Euler angles for each quaternion.
    """
    ...
    sinp = 2 * (w * y - z * x)
    pitch = torch.where(torch.abs(sinp) >= 1, torch.sign(sinp) * torch.tensor(math.pi / 2), torch.asin(sinp))
    ...
    return torch.stack((roll, pitch, yaw), axis=1)