...
class FLAMEMsgFromNPY:
    ...
    def __init__(self, npy_arg, npy_folder='flame', every_x_frames=1, reset_frames=0, smooth=True, preprocess_model=None):  # client
        ...
        self.smooth = smooth
        self.y_preprocess = preprocess_model
    ...
    async def msg_from_npy(self, npy_group):
        ...
        for p0_fp in npy_group:
            ...
            tmp_Y = p0_deca.astype(np.float32)[:,:,:56]


            if self.smooth:
                tmp_Y = self.bilateral_filter(tmp_Y)

            ## standardize dataset
            self.standardize_speaker_data(tmp_Y)

            count_b = 0
            for batch in range(tmp_Y.shape[0]):
                if test_Y is None:
                    #print("set initial batch")
                    test_Y = tmp_Y[batch,:,:]
                else:
                    #print(f"count {count_b}")
                    test_Y = np.concatenate((test_Y, tmp_Y[batch,:,:]), axis=0)
                count_b += 1



            if test_Y.shape[0] > df_au_row_count:
                df_au_row_count = test_Y.shape[0]
    ...
    def standardize_speaker_data(self, data_Y):
        if self.y_preprocess is not None:
            body_mean_Y = self.y_preprocess['body_mean_Y']
            body_std_Y = self.y_preprocess['body_std_Y']
            #body_mean_audio = self.preprocess['body_mean_audio']
            #body_std_audio = self.preprocess['body_std_audio']

            data_Y = (data_Y - body_mean_Y) / body_std_Y
            print(f"normalized data: {data_Y.shape}")
            return data_Y
        else:
            import warnings
...
if __name__ == '__main__':
    ...
    with open(local_config['base_dir']+local_config['l_vqconfig']) as f:
    ...
    parser.add_argument("--module_id", default="listener_flame_streamer",
    ...
    parser.add_argument("--pub_ip", default=local_config["listener_flame_streamer"]["ip"],
                        help="IP (e.g. 192.168.x.x) of where to pub to; Default: 127.0.0.1 (local)")
    parser.add_argument("--pub_port", default=local_config["listener_flame_streamer"]["port"],
                        help="Port of where to pub to; Default: 5567")
    parser.add_argument("--pub_key", default=local_config["listener_flame_streamer"]["topic"],
    ...
    print(local_config["listener_flame_streamer"])
    parser.add_argument("--npy_arg", default="p1_speak_faces_clean_deca",
    ...
    parser.add_argument("--npy_folder", default=local_config["base_dir"]+local_config["listener_flame_streamer"]["stream_data"],
    ...
    # machine learning specific for listner
      # machine learning specific for listner
    parser.add_argument("--model_path", default=os.path.join(local_config['base_dir'],'vqgan/',
                                                             l_vqconfig['model_path']),
                        help="machine learning model for speaker classification")
    parser.add_argument("--tag", default=l_vqconfig['tag'],
